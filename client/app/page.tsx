...
export default function Home() {
  ...
  const onAddNewNote = useCallback(
    (note: INote) => {
      setNotes((prev: INote[]) => (prev ? [...prev, note] : [note]));
      refetch();
    },
    [refetch]
  );
  ...
  if (user) {
    content = (
      <div className="flex">
        <Sidebar
          allNotes={notes}
          onNoteClickHandler={onNoteClick}
          onNewNoteHandler={onNewNoteClick}
          onDeleteNoteHandler={onDeleteNote}
          noteOpen={noteOpen}
          noteOpenHandler={onNoteOpen}
          noteId={noteId}
          refetchNotes={refetchNotesHandler}
        />
        <Rightbar
          allNotes={notes}
          noteId={noteId}
        ...
        {user && addNoteOpen && (
          <AddNotePage
            AddNewNoteHandler={onAddNewNote}
            AddNoteBackHandler={onAddNoteBackHandler}
          />
        )}
      </div>
    );
